...
class Grid(object):
    ...
    def neighbors(self, node, diagonal_movement=DiagonalMovement.never):
        ...
        # ↑
        if self.walkable(x, y - 2):
            neighbors.append(self.nodes[y - 2][x])
            s0 = True
        # →
        if self.walkable(x + 2, y):
            neighbors.append(self.nodes[y][x + 2])
            s1 = True
        # ↓
        if self.walkable(x, y + 2):
            neighbors.append(self.nodes[y + 2][x])
        ...
        # ↖
        if d0 and self.walkable(x - 2, y - 2):
            neighbors.append(self.nodes[y - 2][x - 2])

        # ↗
        if d1 and self.walkable(x + 2, y - 2):
            neighbors.append(self.nodes[y - 2][x + 2])

        # ↘
        if d2 and self.walkable(x + 2, y + 2):
            neighbors.append(self.nodes[y + 2][x + 2])

        # ↙
        if d3 and self.walkable(x - 2, y + 2):
            neighbors.append(self.nodes[y + 2][x - 2])
